...
Once `orca-ide` is available, use Tailmux's [host setup workflow](/docs/setup) to review and create the systemd user service:
...
Review mode changes nothing. `--apply` writes the unit but leaves it stopped and disabled so you can install and persist the firewall rule first. Existing units require the explicit `--replace` option and receive a timestamped backup.
...
The recommended path is `tailmux setup orca`, described above. It discovers the absolute `orca-ide` path, creates the private runtime directory and writes this unit safely. The manual equivalent is included for hosts where the setup command cannot manage systemd.
