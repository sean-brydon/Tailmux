...
forwards.json        Saved forward definitions
loopbacks.json       Dedicated per-box loopback addresses and names
loopbacks.lock       Serializes loopback assignment updates
terminal/
...
The second command starts a new daemon automatically. A stop closes active local transports but preserves the identity files and saved forward definitions. Herdr and dedicated tmux shells continue on remote hosts. Forwards save automatically and restore when networking starts. On macOS, loopback setup installs a boot job to restore dedicated address aliases.
